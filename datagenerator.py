...
def loadImages(directory, N, H, W, dims=(32,32,3)): 
    ...
    if len(glob.glob(cat_files)) == 0:  
        ...
        print("Returning {0} numbers per cat".format(num_per_cat))
        img_names = []
        #print(num_per_cat)
        ...
        for dirname in sorted(os.listdir(directory)):
            ...
            filenames = [os.path.join(path, fname) for fname in sorted(os.listdir(path))][:num_per_cat]
 
            cats.extend([dirname] * len(filenames))
            fnames.extend(filenames)
            img_names.extend(filenames)
        #print(len(cats), len(fnames))
        #print(cats, fnames)
        #assert(np.unique(cats).shape[0] == len(categories))
        print(np.unique(fnames).shape[0], np.unique(fnames).shape[0] == NUM_TRAIN)
        
        #assert(np.unique(fnames).shape[0] == NUM_TRAIN)
        print("Following Categories")
...
#print(fnames_train, cats_train)

#for i in np.arange(cats_train.shape[0]): 
#    print(np.argmax(cats_train[i]), categories[np.argmax(cats_train[i])], fnames_train[i])
    #assert(categories[np.argmax(cats_train[i])] in fnames_train[i])
